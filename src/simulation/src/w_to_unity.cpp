...
void wCallback(const mav_msgs::Actuators& cmd)
{
  ...
  Arr[4]=cmd.angular_velocities[4];

  std::cout <<"Message received: "<< Arr[0] << " "  << Arr[1] << " "<< Arr [2] << " "  << Arr[3] << " "  << Arr[4] << " " << "\n";
...
int main(int argc, char **argv)
{
  ...
  float counter=0;
  ros::Subscriber sub = n.subscribe("commands", 1, wCallback);
